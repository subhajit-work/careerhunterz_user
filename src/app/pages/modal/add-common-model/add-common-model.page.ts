...
export class AddCommonModelPage implements OnInit, OnDestroy {
  ...
  getQuestionSubscribe: Subscription;
  getQuestion_api;
  getCountry_api;
  country: any;
  getCountrySubscribe: Subscription;
  // @Input() identifire;
  ...
  ngOnInit() {
    ...
    if(this.get_identifier == 'signUp')
    {
      ...
      this.onEditField = 'POST';
      this.api_url = 'register';
      // getservice api
      this.getService_api = 'getservice';
      this.getService();
      // getQuestion api
      this.getQuestion_api = 'getquestion';
      this.getQuestion();
      ...
      this.commonUtils.signinStudentInfoObservable.pipe(
        ...
        if(res){
          ...
          // ================== view data fetch start =====================
            this.editDataSubscribe = this.http.get('get-user').subscribe(
              (res:any) => {
    ...
    }else if(this.get_identifier == 'rating'){
      this.heder_title = 'Rating & Reviews';
      this.onEditField = 'PUT';
      this.api_url =  'student/forget_password';
    }
...
getService(){
  ...
  this.getServiceSubscribe = this.http.get(this.getService_api).subscribe(
    (res:any) => {
    this.selectLoadingDepend = false;
    this.service =res.return_data;
    console.log('sss', res);
  },
  errRes => {
    this.selectLoadingDepend = false;
...
getQuestion(){
  this.selectLoadingDepend = true;
  this.getQuestionSubscribe = this.http.get(this.getQuestion_api).subscribe(
    (res:any) => {
    this.selectLoadingDepend = false;
    this.question =res.return_data;
  ...
  getServiceContact_api(getServiceContact_api: any) {
...
// export function call end
  ...
  ngOnDestroy(){
    ...
    if(this.getQuestionSubscribe !== undefined){
      this.getQuestionSubscribe.unsubscribe();
    }
  }
