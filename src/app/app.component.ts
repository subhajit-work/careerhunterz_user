...
export class AppComponent implements OnInit {
  ...
  menus = [
    ...
    },
    {
      pages: [
        {
          value: {
            page_display_name: "Blockchain Technology",
            page_icon: "",
            page_id: "90",
            page_name: "continuous-learning?interest_id=38",
            page_url: "/continuous-learning?interest_id=38&blockchain-technology"
          }
        },
        {
          value: {
  ...
  userInfoData(){
    ...
    .pipe(
      ...
      if(baseUrl == 'localhost' || baseUrl == '192.168.0.10'){
        this.site_url_name = 'https://goldenventure.nichetechnosolution.com';
      }else{
      ...
      this.userInfoSubscribe = this.http.get('footer').subscribe(
        (response:any) => {
        this.userInfodDataLoading = false;
  ...
  userDataGet(){
    this.userDataSubscribe = this.http.get('get-user').pipe(
      take(1)
      ).subscribe(
      (res:any) => {
        ...
        if(res.return_status > 0){
          ...
          // user details set
          this.commonUtils.onSigninStudentInfo(res.return_data.user_info);
        }
      },
      errRes => {
        // this.viewLoadData = false;
  ...
  goToPage(_url, _item){
    ...
    let mainUrl;
    if(_url == 'service'){
      mainUrl = _url+'/'+_item.slug+'/'+_item.id
    }else {
      mainUrl = _url+'/'+_item
    }
    
    this.navCtrl.navigateRoot(mainUrl);
